...
def make_fig1(
    ...
    suptitle: str = "Performance of GaWF RNN on the CT-MNIST task",
    suptitle_y: float = 1.05,
    suptitle_fs: float = 14.0,
    gawf_only: bool = False,
) -> None:
    results = load_results_pkl(gawf_pkl)

    if gawf_only:
        fig, axes = plt.subplots(1, 2, figsize=figsize if figsize != (13.5, 3.8) else (9.0, 3.8))
...
def main() -> None:
    ...
    p.add_argument(
        "--gawf_only",
        action="store_true",
        help="Only plot GaWF char/pos accuracy curves (2-panel); skip the bar chart.",
    )
    args = p.parse_args()
