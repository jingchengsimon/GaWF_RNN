...
Sector mode  (--sector S  --agg {space|feature}):
  For each frame where sector_label == S, accumulate:
    gate_ih  = sigmoid(U @ (fb * V_ih) / tau)        shape (H, input_size)
    outer_ih = U[:,nc+S] (outer) V_ih[nc+S,:] * fb[nc+S]  — no sigmoid
  Then aggregate along the input dimension (input_size = 32 * 6 * 6 = 1152):
    space   → mean over 32 feature channels → shape (36, H)   [row=spatial, col=hidden]
    feature → mean over 6×6 spatial grid   → shape (32, H)   [row=feature, col=hidden]
  Also saves the aggregated W_ih (static weights, same shape) once per (agg mode).
  Outputs in --save_dir:
...
from anal_utils.export_gate_sample import build_model_from_ckpt, build_test_dataset
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "Sector mode only — how to aggregate the 1152-d input axis:\n"
            "  space   → mean over 32 feature channels → (36, H)  [spatial positions]\n"
            "  feature → mean over 6×6 spatial grid   → (32, H)  [feature channels]"
        ),
    )
    parser.add_argument(
        "--save_dir",
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="Temperature tau for gate computation (default: 2.0, same as middle_gawf).",
    )
    parser.add_argument(
        "--device",
        type=str,
...
def compute_avg_gate_ih(
    ...
    """
    Iterate all test samples. For each frame where sector_label == sector,
    compute gate_ih and the rank-1 outer product for the sector basis component.

    gate_ih[i, j] = sigmoid(trans_ih[i, j] / tau)
    where trans_ih = U @ (fb * V_ih),  shape (H, input_size).

    outer_ih = U[:, nc+s] outer V_ih[nc+s, :] * fb[nc+s],  shape (H, input_size).

    Returns:
        avg_gate_ih  : (H, input_size) float32
        avg_outer_ih : (H, input_size) float32, no sigmoid
        n_frames     : total qualifying frames accumulated
        n_samples    : samples containing at least one qualifying frame
    """
    H         = model.rnn.hidden_size
    fb_dim    = model.num_classes + model.num_pos
    nc        = model.num_classes              # feedback offset for sector
    d_sector  = nc + sector                    # feedback vector index for this sector
    ...
    with torch.no_grad():
        for sidx in range(n_total):
            ...
            sector_ids = labels_np[:, 1].astype(np.int64)
            t_indices  = np.where(sector_ids == sector)[0]
            ...
            for t in t_indices:
                ...
                # Step 2: recompute gate_ih with updated feedback
                fb1      = prev_fb.clamp(-10, 10).unsqueeze(2)   # (1, fb_dim, 1)
                ...
                # Rank-1 outer product for this sector's basis component (no sigmoid)
                u_d     = model.U[:, d_sector]          # (H,)
                v_ih_d  = V_ih[0, d_sector, :]          # (input_size,)
                fb_d    = fb1[0, d_sector, 0]           # scalar
    ...
    if n_frames == 0:
        raise RuntimeError(
            f"No frames with sector={sector} found in test set."
        )
...
def main() -> None:
    ...
    if args.digit is not None:
        ...
        gate_path  = os.path.join(args.save_dir, f"avg_gate_hh_{fg_digit}.npy")
        outer_path = os.path.join(args.save_dir, f"avg_outer_hh_{fg_digit}.npy")
        meta_path  = os.path.join(args.save_dir, f"avg_gate_meta_{fg_digit}.json")
        ...
        _save_digit_boundaries(args.hidden_data_dir, args.save_dir)
    ...
    # ---------------------------------------------------------------- sector mode
        ...
        avg_gate_ih, avg_outer_ih, n_frames, n_samples = compute_avg_gate_ih(
            test_ds, model, device, tau, sector
        )
        print(
            f"Done.  n_samples={n_samples}  n_frames={n_frames}\n"
        ...
        gate_path  = os.path.join(args.save_dir, f"avg_gate_ih_{tag}.npy")
        outer_path = os.path.join(args.save_dir, f"avg_outer_ih_{tag}.npy")
        wih_path   = os.path.join(args.save_dir, f"weight_ih_{agg}.npy")
        meta_path  = os.path.join(args.save_dir, f"avg_gate_meta_{tag}.json")
