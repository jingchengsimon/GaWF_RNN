% Include this file from main.tex with \input{appendix}.
% The manuscript preamble must load graphicx and its usual math/citation packages.
% Table floats are maintained separately under tables/.
% Source values follow docs/GaWF_STATS_RECORD.md (working draft dated 2026-09-15).
...
Parameter matching is defined over the complete trainable model, including the shared encoder, recurrent core, and both readout heads. Table~\ref{tab:formal-config} reports the selected primary width, total trainable parameter count, and formal optimizer settings of each model. For RNN, LSTM, GRU, and GaWF, the reported width denotes the recurrent hidden size \(H\). For Mamba and S5, it denotes the model dimension \(d_{\mathrm{model}}\). Mamba uses a fixed state dimension \(d_{\mathrm{state}}=16\), whereas S5 uses \(d_{\mathrm{state}}=128\). The width entries should therefore be interpreted according to each architecture rather than as a common internal state dimension. All architectures instantiate the same encoder class and use the same PyTorch initialization rules, but their parameter tensors are independently initialized. RNN, LSTM, GRU, Mamba, and S5 receive no output-feedback pathway.

The Mamba baseline uses one Mamba-v1 block with convolution width 4, expansion factor 2, a residual connection, zero inter-layer dropout, and output dropout \(0.5\). The S5 baseline uses one S5 layer with a residual connection, zero inter-layer dropout, and output dropout \(0.5\). No model uses a bidirectional recurrent core.

\section{Hyperparameter selection and data-scale comparison}
\label{app:hparams}

Hyperparameters were selected with seed 42 on the fixed 40-hour validation split, using at most 100 epochs and early stopping with patience 15. For GaWF, we searched \(H\in\{64,128,256,512\}\), learning rate \(\{10^{-4},5\times10^{-4},10^{-3},5\times10^{-3}\}\), and weight decay \(\{0,10^{-5},10^{-4},10^{-3}\}\); after parameter matching fixed each baseline width, RNN, LSTM, and GRU used the same learning-rate and weight-decay sets, while Mamba and S5 added \(10^{-2}\) to the learning-rate set. Every candidate received the same per-run training budget, but the recorded grids did not contain the same number of configurations per architecture: the 40-hour GaWF search evaluated 64 configurations, RNN/LSTM/GRU evaluated 16 each, and Mamba/S5 evaluated 20 each. In every case, the configuration maximizing validation target-identity accuracy was selected and then retrained with seeds 1--10 for 150 epochs without early stopping. The selected 40-hour GaWF configuration and the model-specific baseline configurations in Table~\ref{tab:formal-config} were frozen across the 4-, 10-, 20-, and 40-hour comparisons. Figure~\ref{fig:data-scale} reports training accuracy, validation accuracy, and their difference at the selected checkpoint; points and error bars are the mean and seed-level SEM over the ten formal runs.

% Pending Amarel check: confirm the recorded Mamba and S5 totals by runtime instantiation.
\input{tables/01_formal_model_configurations}
...
Canonical six-model accuracy uses the standard test movie with 32-frame rollouts, whereas the main GaWF feedback-shuffle analysis uses the same movie with 512-frame rollouts. Their absolute accuracies can therefore differ because the recurrent state is reset at different frequencies, not because they use different datasets. The six-model switch-recovery analysis uses the joint-balanced test movie with 32-frame rollouts, and the supplementary feedback-removal recovery analysis uses that movie with 512-frame rollouts. Every protocol excludes the first output of each recurrent window.
...
With two input channels and 32 recurrent outputs per window, the dataset length is \(\lfloor(N_{\mathrm{raw}}-2)/32\rfloor\). An epoch contains \(\lfloor\lfloor(N_{\mathrm{raw}}-2)/32\rfloor/256\rfloor\) optimizer updates because the final incomplete batch is dropped and gradient accumulation is disabled. The only change across scales is the duration of the training movie; validation data and all other formal settings remain fixed.
...
GaWF achieved the highest accuracy on both task variables under the canonical 32-frame evaluation protocol (target identity, \(86.6\%\pm0.1\%\); target location, \(93.3\%\pm0.1\%\)). Mamba was the closest baseline on both readouts (target identity, \(83.1\%\pm0.2\%\); target location, \(92.6\%\pm0.1\%\)). Every reported accuracy excludes the window-initial output and is first computed within a training seed before aggregation across seeds.
...
The shared encoder produced a location-dominated but strongly conjunctive representation, whereas GaWF hidden activation shifted toward target identity with a smaller interaction component (Figure~\ref{fig:activation-condition-variance}). At synapse level, the input gate was dominated by target location and the recurrent gate by target identity (input gate: location \(76.7\%\pm0.5\%\), identity \(16.0\%\pm0.4\%\), interaction \(7.3\%\pm0.1\%\); recurrent gate: location \(23.3\%\pm0.5\%\), identity \(71.7\%\pm0.5\%\), interaction \(5.0\%\pm0.1\%\)). These results indicate a complementary assignment of task variables to the input and recurrent pathways.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{results/save/iclr_figs/Appendix_fig2_activation_anova_1x2_6model_10seed.pdf}
\caption{Condition-mean variance decomposition of input and hidden activations across the six models. Each panel reports the target-location, target-identity, and sector-by-digit interaction fractions, normalized to sum to \(100\%\) within each model and representation. Bars and error bars denote the mean and seed-level SEM over ten independently trained models; each gray point represents one training seed. Trial-level residual variance is excluded from this decomposition.}
\label{fig:activation-condition-variance}
\end{figure*}

For comparison with unit-level LSTM and GRU gates, we project each GaWF gate to its destination unit by taking the arithmetic mean over all incoming synaptic gates. Conventional gates also reflect both task variables, but their dominant fractions are less pronounced. This comparison does not isolate connection-level granularity from the use of output-derived feedback.
...
\input{tables/14_input_gate_sign}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{results/save/iclr_figs/Supple2_input_gate_sign_vs_mag_sector_delta_zoom_10seed_9sector.pdf}
\caption{Input-gate modulation as a function of static weight magnitude. Positive- and negative-weight connections are shown separately for location-matching and other encoder sources. Faint points are a display-only subsample of pooled connection--condition observations; curves show nine-bin means and seed-level SEM across ten training seeds. Seed-level gap and slope summaries are reported in the text.}
...
\input{tables/15_recurrent_sign_gap}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{results/save/iclr_figs/Supple3_rec_gate_sign_vs_mag_digit_sector_delta_2x4_10seed.pdf}
\caption{Recurrent-gate modulation as a function of static recurrent-weight magnitude. The first row shows digit conditions and the second row shows sector conditions; columns show \(T\!\to\!T\), \(T\!\to\!R\), \(R\!\to\!T\), and \(R\!\to\!R\). Faint points are a display-only subsample of pooled connection--condition observations; curves show nine-bin means and seed-level SEM across ten training seeds. Slopes are fitted separately within each training seed over its shared positive/negative \(|W|\) support and summarized across seeds in the text.}
...
\input{tables/16_recurrent_current}
