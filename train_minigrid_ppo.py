...
from utils.atari_train_utils import compute_gae, ensure_dir, save_json, select_device, set_atari_seed
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--env_id", type=str, default="MiniGrid-MemoryS7-v0", choices=MINIGRID_PILOT_ENVS)
...
def train(args: argparse.Namespace) -> dict:
    ...
    try:
        ...
        def encoder_factory():
            return MiniGridEncoder(output_size=args.encoder_output_size, encoder_type=args.encoder,
                                   grid_size=int(next_obs.shape[-1]), hidden_size=args.encoder_hidden)

        model = MiniGridActorCritic(
            num_actions=num_actions, encoder=encoder_factory(), model_type=args.model_type,
            hidden_size=args.hidden_size, core_dropout=args.core_dropout,
            ssm_d_model=args.ssm_d_model, ssm_state_size=args.ssm_state_size,
            ssm_num_layers=args.ssm_num_layers, ssm_context_len=args.num_steps,
        ).to(device)
        ...
        for update in range(1, num_updates + 1):
            ...
            for step in range(T):
                ...
                with torch.no_grad():
                    action, logprob, _ent, value, state = model.act(next_obs, next_done, state=state)
            ...
            with torch.no_grad():
                _a, _lp, _e, next_value, _s = model.act(next_obs, next_done, state=state, deterministic=True)
                advantages, returns = compute_gae(rewards_buf, dones_buf, values_buf,
            ...
            for _epoch in range(args.update_epochs):
                ...
                optimizer.step()
                last_pl = float(policy_loss.detach()); last_vl = float(value_loss.detach())
            ...
            if update % args.log_interval_updates == 0 or update == num_updates:
                ...
                logger.info("upd=%d/%d step=%d success=%.3f return=%.3f pl=%.4f vl=%.4f ent=%.3f fps=%d",
                            update, num_updates, global_step, success, mean_ret,
                            last_pl, last_vl, last_ent, fps)
                with open(history_path, "a", encoding="utf-8") as f:
                    f.write(json.dumps({"global_step": global_step, "update": update,
                                        "success_rate": success, "episodic_return_100": mean_ret,
                                        "policy_loss": last_pl, "value_loss": last_vl,
                                        "entropy": last_ent, "fps": fps}) + "\n")

        final_metrics = {"env_id": args.env_id, "algo": args.algo, "model_type": args.model_type,
                         "encoder": args.encoder, "global_step": global_step,
                         "success_rate": success, "episodic_return_100": mean_ret}
        ckpt = os.path.join(save_dir, f"ppo_{args.model_type}_{args.env_id.replace('/', '_')}.pth")
