#!/usr/bin/env bash
# Phase 3 — final runs (requires artifacts/phase2_final_hparams.json from Phase 2).
# Two GPUs: wave 1 (rnn + lstm), wave 2 (gru + gawf).
set -euo pipefail
...
COMMON=(--data_suffix "$DS" --eval_data_suffix 40h-float32 --cnn_dropout 0.0 --rnn_dropout 0.5
  --num_epochs 100 --patience 15 --use_acceleration --use_sector_mode)

CUDA_VISIBLE_DEVICES=0 python train_model.py "${COMMON[@]}" \
  --model_types rnn --hidden_sizes 275 \
  --lrs "$RNN_LR" --wds "$RNN_WD" \
  --result_suffix "gen_phase3_${SK}_rnn" &
CUDA_VISIBLE_DEVICES=1 python train_model.py "${COMMON[@]}" \
  --model_types lstm --hidden_sizes 80 \
  --lrs "$LSTM_LR" --wds "$LSTM_WD" \
  --result_suffix "gen_phase3_${SK}_lstm" &
wait

CUDA_VISIBLE_DEVICES=0 python train_model.py "${COMMON[@]}" \
  --model_types gru --hidden_sizes 105 \
  --lrs "$GRU_LR" --wds "$GRU_WD" \
  --result_suffix "gen_phase3_${SK}_gru" &
CUDA_VISIBLE_DEVICES=1 python train_model.py "${COMMON[@]}" \
  --model_types gawf --hidden_sizes 256 \
  --nofb --fb_start_epoch 50 \
  --lrs "$GAWF_LR" --wds "$GAWF_WD" \
  --result_suffix "gen_phase3_${SK}_gawf" &
wait

python experiments/generalization/collect_results.py phase3 --scale "$SK" \
  "results/train_data/gen_phase3_${SK}_rnn" \
  "results/train_data/gen_phase3_${SK}_lstm" \
  "results/train_data/gen_phase3_${SK}_gru" \
  "results/train_data/gen_phase3_${SK}_gawf"
