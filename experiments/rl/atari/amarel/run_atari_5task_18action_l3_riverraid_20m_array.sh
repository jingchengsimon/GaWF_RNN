...
[[ "$END_EPSILON" == "0.05" ]] || {
  echo "END_EPSILON must be 0.05 for the Riverraid C protocol" >&2
  exit 2
}
...
python -m experiments.rl.atari.amarel.scratch_quota_guard \
  ...
  --required_gib "${REQUIRED_GIB:-65.8}" --headroom_factor "${QUOTA_HEADROOM_FACTOR:-2}" \
...
DISABLE_TQDM=1 python run_task.py atari-dqn \
  ...
  --start_epsilon 1.0 --end_epsilon "$END_EPSILON" \
