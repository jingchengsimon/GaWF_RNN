#!/usr/bin/env bash
#SBATCH --job-name=aim3-param-match
...
#SBATCH --output=experiments/amarel/artifacts/hparam_param_match/%A_%a.out
#SBATCH --error=experiments/amarel/artifacts/hparam_param_match/%A_%a.err

# Run one param-count matched RNN/GRU/LSTM hparam task.
...
ART_ROOT="$ROOT/experiments/amarel/artifacts/hparam_param_match"
STATUS_DIR="$ROOT/experiments/generalization/artifacts/gen_hparam_param_match/status"
mkdir -p "$ART_ROOT" "$STATUS_DIR"

GAWF_REF_HIDDEN="${GAWF_REF_HIDDEN:-512}"
...
DONE_FILE="$STATUS_DIR/matchgawf${GAWF_REF_HIDDEN}_task_$(printf '%04d' "$TASK_ID").done"
FAIL_FILE="$STATUS_DIR/matchgawf${GAWF_REF_HIDDEN}_task_$(printf '%04d' "$TASK_ID").fail"
...
eval "$(python "$GRID_UTIL" emit-task --task-id "$TASK_ID" --root "$ROOT" --gawf-ref-hidden "$GAWF_REF_HIDDEN")"

echo "[$(date -Is)] task_id=$TASK_ID scale=$SCALE model=$MODEL_TYPE h=$HIDDEN_SIZE match_gawf=$GAWF_REF_HIDDEN lr=$LR wd=$WD"
echo "data_dir=$DATA_DIR"
echo "result_suffix=$RESULT_SUFFIX"
echo "metrics_path=$METRICS_PATH"

if python "$GRID_UTIL" validate --task-id "$TASK_ID" --root "$ROOT" --gawf-ref-hidden "$GAWF_REF_HIDDEN" >/dev/null 2>&1; then
...
if python "$GRID_UTIL" validate --task-id "$TASK_ID" --root "$ROOT" --gawf-ref-hidden "$GAWF_REF_HIDDEN" --json; then
  {
    echo "status=done"
    echo "task_id=$TASK_ID"
    echo "scale=$SCALE"
