#!/usr/bin/env bash
# Train the chan=1 six-model/ten-seed protocol on two GPUs, then run balanced testing.

set -u -o pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="${AIM3_ROOT:-$(cd "$SCRIPT_DIR/../.." && pwd)}"
GRID_UTIL="$ROOT/experiments/generalization/clutter_best6_chan1_multiseed.py"
CONDA_INIT="${AIM3_CONDA_INIT:-/home/${USER}/enter/etc/profile.d/conda.sh}"
...
mkdir -p "$ARTIFACT_DIR/status" "$ANALYSIS_ROOT"
...
run_one() {
  local task_id="$1"
  local gpu="$2"
  local analysis_dir ckpt_tag analysis_npz analysis_meta rc

  eval "$(python "$GRID_UTIL" emit-task --task-id "$task_id" --root "$ROOT")"
  analysis_dir="$ANALYSIS_ROOT/$UNIT_ID"
  ckpt_tag="$(basename "$CHECKPOINT_PATH" _model.pth)"
  analysis_npz="$analysis_dir/fg_switch_offset_acc_${ckpt_tag}.npz"
  analysis_meta="$analysis_dir/fg_switch_offset_meta_${ckpt_tag}.json"
  mkdir -p "$analysis_dir" "$(dirname "$DONE_FILE")"
  ...
  echo "[$(date -Is)] gpu=$gpu task=$TASK_ID unit=$UNIT_ID phase=balanced_test"
  if [[ ! -s "$analysis_npz" || ! -s "$analysis_meta" ]]; then
    CUDA_VISIBLE_DEVICES="$gpu" python utils_anal/export_fg_switch_offset_acc.py \
      --switch_target fg \
      ...
      --seed "$SEED"
    rc=$?
    if [[ $rc -ne 0 || ! -s "$analysis_npz" || ! -s "$analysis_meta" ]]; then
      printf 'status=failed\nphase=balanced_test\nexit_code=%s\ntask_id=%s\nunit_id=%s\ntimestamp=%s\n' \
        "$rc" "$task_id" "$UNIT_ID" "$(date -Is)" > "$FAIL_FILE"
      return 1
    fi
...

  {
    ...
    echo "analysis_npz=$analysis_npz"
    echo "analysis_meta=$analysis_meta"
...
run_worker() {
  ...
  local gpu="$2"
  local task_id
  for ((task_id = worker; task_id < 60; task_id += 2)); do
