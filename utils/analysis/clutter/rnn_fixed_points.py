...
Uses the trained nn.RNN directly; autonomous analysis overrides only bias_ih and supplies zero
features, removing the complete input pathway while preserving recurrent bias. No training.
"""
...
def step(rnn: torch.nn.RNN, h: torch.Tensor, inputs: torch.Tensor,
         autonomous: bool = False) -> torch.Tensor:
    """Advance the canonical RNN with the selected feedforward intervention."""
    if autonomous:
        inputs = torch.zeros_like(inputs)
        _, next_h = functional_call(rnn, {"bias_ih_l0": torch.zeros_like(rnn.bias_ih_l0)},
                                   (inputs[:, None], h[None]))
    else:
...
def search(rnn: torch.nn.RNN, initial: torch.Tensor, inputs: torch.Tensor,
           autonomous: bool, iterations: int, lr: float) -> tuple[np.ndarray, np.ndarray]:
...
def characterize(rnn: torch.nn.RNN, candidates: np.ndarray, inputs: torch.Tensor,
                 autonomous: bool, tolerance: float, merge_rms: float) -> dict[str, np.ndarray]:
    ...
    for h0 in candidates_t:
        ...
        perturb = torch.as_tensor(rng.normal(size=(8, dimension)), device=h.device,
                                 dtype=torch.float64)
        ...
        rel_error = ((actual - base - predicted_delta).norm(dim=1)
                     / (actual - base).norm(dim=1).clamp_min(1e-15)).mean()
    ...
    return {"roots": np.asarray(roots, dtype=np.float32).reshape(-1, dimension),
...
def run(args: argparse.Namespace) -> None:
    ...
    rnn = model.core.rnn
    if not isinstance(rnn, torch.nn.RNN) or rnn.num_layers != 1 or rnn.nonlinearity != "tanh":
        raise ValueError("Requires a one-layer tanh RNN")
    ...
    previous = torch.tensor(trajectories[:, [zero - 2, zero - 1]].reshape(-1, rnn.hidden_size),
                            device=device)
    ...
    np.savez_compressed(args.save_dir / "inputs_and_initializations.npz",
                        anchor_frames=anchors.astype(np.int64),
                        features=features.cpu().numpy().astype(np.float32),
                        initial=initial.astype(np.float32), pool_indices=sampled.astype(np.int64))
    summaries = []
    for index, feature in enumerate(inputs):
        ...
        candidates, q = search(rnn, initial_t, condition_input, autonomous, args.iterations, args.lr)
        result = characterize(rnn, candidates, condition_input, autonomous,
                              args.residual_tolerance, args.merge_rms)
        np.savez_compressed(args.save_dir / f"{name}.npz", candidates=candidates, q=q, **result)
        spectral = result["spectral_radius"]
        summaries.append({"condition": name, "roots": len(spectral),
    ...
    metadata = {"seed": args.seed, "checkpoint": str(args.ckpt),
                "event_dir": str(args.event_dir), "movie": str(args.movie),
                ...
                "starts": args.starts, "iterations": args.iterations,
                "residual_tolerance_max_absolute": args.residual_tolerance,
                "merge_rms": args.merge_rms, "conditions": summaries,
