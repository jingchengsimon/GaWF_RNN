...
class GaWFCoreLegacy(GaWFDiagnosticsMixin, nn.Module):
    ...
    def __init__(
        ...
        if rnn_activation not in ("tanh", "identity"):
            raise ValueError(f"Unsupported rnn_activation: {rnn_activation!r}")
        if (output_wrap != "ln_relu_dropout" or rnn_activation != "tanh") and num_layers != 1:
            raise ValueError("GaWF wrap/activation variants require num_layers == 1")
    ...
    def _apply_inner_activation(self, preactivation: torch.Tensor) -> torch.Tensor:
        """Apply the configured state activation inside the recurrence."""

        if self.rnn_activation == "identity":
            return preactivation
        return torch.tanh(preactivation)

    def _apply_wrap(self, h_t: torch.Tensor, norm: nn.Module | None) -> torch.Tensor:
        """Apply the configured outer LayerNorm, ReLU, and dropout wrap."""
    ...
    def project_readout(self, h_t: torch.Tensor) -> torch.Tensor:
        """Return the readout for a state.

        This historical core already wrapped the state inside the loop, so the state *is* the
        readout and this is the identity. The method exists so task wrappers can address both core
        generations through one interface.
        """
    ...
    def _step_layer(
        ...
        norm = self.norm if self.num_layers == 1 else (
            self.norms[layer_idx] if self.norms is not None else None
        )
        ...
        return self._apply_wrap(self._apply_inner_activation(preactivation), norm)
    ...
    def step_no_feedback(
        ...
        if self.num_layers == 1:
            ...
            return self._apply_wrap(self._apply_inner_activation(ih + hh), self.norm)
        ...
        for idx, (rnn, norm) in enumerate(zip(self.rnns, norms)):
            ih = F.linear(layer_input, rnn.weight_ih_l0, rnn.bias_ih_l0)
            hh = F.linear(state[idx], rnn.weight_hh_l0, rnn.bias_hh_l0)
            layer_input = self._apply_wrap(self._apply_inner_activation(ih + hh), norm)
            next_states.append(layer_input)
        return layer_input, next_states

    def forward_no_feedback(self, x: torch.Tensor):
        if self.num_layers == 1:
            if self.output_wrap == "ln_relu_dropout" and self.rnn_activation == "tanh":
                out, state = self.rnn(x)
                out = self.norm(out)
                out = F.relu(out)
                out = F.dropout(out, p=self.dropout, training=self.training)
                return out, state
            return self._forward_no_feedback_variant(x)
        layer_output = x
        final_states: list[torch.Tensor] = []
        norms = self.norms if self.norms is not None else [None] * self.num_layers
        for rnn, norm in zip(self.rnns, norms):
            layer_output, state = rnn(layer_output)
            layer_output = self._apply_wrap(layer_output, norm)
            final_states.append(state.squeeze(0))
        return layer_output, final_states

    def _forward_no_feedback_variant(self, x: torch.Tensor):
        """Per-step ungated rollout under the configured activation and wrap settings."""
...
def configure_gawf_feedback_acceleration_legacy(
    module: nn.Module,
    enabled: bool,
    compile_mode: str = "reduce-overhead",
) -> int:
    """Configure every nested :class:`GaWFCoreLegacy` feedback subgraph.
    ...
    for child in module.modules():
        if not isinstance(child, GaWFCoreLegacy):
            continue
