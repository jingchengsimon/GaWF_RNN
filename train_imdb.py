...
def count_core_params(model: nn.Module) -> int:
    """Trainable params excluding the shared embedding and the shared head (fc)."""
    total = 0
    for name, p in model.named_parameters():
        if not p.requires_grad:
            continue
        if name.startswith("embedding.") or name.startswith("fc."):
            continue
        total += p.numel()
    return total
...
def build_optimizer(model: nn.Module, lr: float, weight_decay: float, optim_name: str):
    """Adam(W) with GaWF U/V excluded from weight decay (mirrors the vision rule)."""
    decay, no_decay = [], []
    for name, p in model.named_parameters():
        if not p.requires_grad:
            continue
        # GaWF feedback params (U, V) and all bias/1-D params get no weight decay.
...
def maybe_subset(loader: DataLoader, max_samples: Optional[int], batch_size: int, shuffle: bool) -> DataLoader:
...
def train_one_config(args, cfg: Dict, loaders: Dict[str, DataLoader], device: str, meta: Dict) -> Dict:
    set_seed(args.seed)
    ...
    for epoch in range(args.num_epochs):
        ...
        train_metrics = {"loss": run_loss / max(run_total, 1), "acc": run_correct / max(run_total, 1)}
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--embed_dropout", type=float, default=0.0, help="Fixed (vision config); not swept.")
    p.add_argument("--rnn_dropout", type=float, default=0.5, help="Fixed (vision config); not swept.")
...
def main() -> None:
    ...
    if args.max_train_samples:
        loaders["train"] = maybe_subset(loaders["train"], args.max_train_samples, args.batch_size, True)
    if args.max_eval_samples:
        for split in ("val", "test"):
            loaders[split] = maybe_subset(loaders[split], args.max_eval_samples, args.batch_size, False)
