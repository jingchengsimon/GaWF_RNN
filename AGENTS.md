# AGENTS.md — aim3_RNN Project Specification
...
Codex instruction. Task registration and result-validity rules live in the shared dashboard
registry: `/Users/jingchengshi/Desktop/MIMO-Rutgers/1-Codes/dashboard/tasks.json`.
...
1. After a successful Amarel Slurm submission or sjc-remote dual-GPU launch, register the task
   in the shared dashboard registry during the **same turn**. Use the centralized path
   `/Users/jingchengshi/Desktop/MIMO-Rutgers/1-Codes/dashboard` (for example:
   `cd "/Users/jingchengshi/Desktop/MIMO-Rutgers/1-Codes" && python3 -m dashboard.manage_tasks ...`).
   Registration is not optional and must not be deferred to a later conversation.
...
5. Remove a task only after a human explicitly confirms that the Dashboard record is no longer
   needed. Use
   `cd "/Users/jingchengshi/Desktop/MIMO-Rutgers/1-Codes" && python3 -m dashboard.manage_tasks remove <id> --human-confirmed`;
   never edit a task away merely because it completed, failed, timed out, or became stale.
...
- Pong DQN defaults to `--frame_skip 1 --frame_stack 1`: every environment step advances
  ...
  `pong_fs1_stack1`, historical `pong_fs4_stack1`, or strict `pong_fs4_stack4`; never use
  `1frame` / `pong1f` as a protocol label.
...
Reusable Amarel launchers and grid utilities may be tracked, but one-off rerun
or training scripts must be clearly marked as one-off in their filename or header
and deleted before synchronizing branches or worktrees. Generated Slurm scripts
belong under `experiments/amarel/generated/` and are not tracked.
...
- `ssh amarel` for Amarel (`js3269@amarel-new.hpc.rutgers.edu`);
  use `ssh amarel-old` only when explicitly checking the legacy host
  (`js3269@amarel.rutgers.edu`)
...
### 9.2 Amarel SSH Session Management

Amarel limits concurrent SSH sessions per user. To avoid exhausting connections:

- **Consolidate queries.** When multiple pieces of information are needed
  (e.g. `sacct` status + log inspection + metrics check), combine them into a
  **single** `ssh amarel '…'` call using `&&` chaining or a heredoc. Never
  issue N separate SSH calls for N related questions.
- **No background SSH sessions.** Always use `is_background=false` for Amarel
  commands. If a command may take long, set a generous `timeout` (up to 120 s)
  rather than falling back to background mode.
- **Stale connection cleanup.** If SSH hangs or returns connection refused,
  run `pkill -f "ssh.*amarel"` locally before retrying.
- **Script-once pattern.** For complex multi-step diagnostics, write the full
  script to a temp file on Amarel (`cat > /tmp/task.sh << 'EOF' … EOF`) in one
  SSH call, then execute it in the same call (`&& bash /tmp/task.sh`).
